    ...
    public async getPullRequestUrl(sha: string, repo: string) {
        ...
        return new Promise((resolve, reject) => {
            this.octokit.search.issues({ q, repo, sort: "created", order: "desc" }, async (error: any, result: any) => {
                let errorMessage = "";
                if (error) {
                    errorMessage = error.code === 422
                        ? "You don't have permission to access. Check your configuration \"vscodetosa.token\" is set and valid."
                        : "Unknown error was occured while searching the Pull Request.";
                ...
                if (items.length === 0) {
                    errorMessage = `Could not find Pull Request. Sha: ${sha}`;
                    // this is not good
